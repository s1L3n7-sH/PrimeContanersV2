...
async function sendQuoteEmail(email: string, name: string) {
    ...
    if (!process.env.SENDGRID_API_KEY) {
        console.warn("[EMAIL] ❌ SENDGRID_API_KEY is not set in environment variables. Email not sent.");
        return;
    }
    ...
    const fromEmail = process.env.SENDGRID_FROM_EMAIL || "rbsuarez00311@usep.edu.ph";
    const replyToEmail = process.env.SENDGRID_REPLY_TO || "rbsuarez00311@gmail.com";

    console.log(`[EMAIL] SendGrid Config: From=${fromEmail}, ReplyTo=${replyToEmail}`);

    const msg = {
        to: email,
        from: {
            email: fromEmail,
            name: "Prime Containers"
        },
    ...
    try {
        console.log(`[EMAIL] Sending email via SendGrid to ${email}...`);
